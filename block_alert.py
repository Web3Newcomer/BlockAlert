...
class BlockAlert:
    def __init__(self, target_height, wecom_config, early_alert_diff):
    ...
    def check_alert(self, current_height):
        ...
        for i in range(1, self.early_alert_diff + 1):
            check_height = self.target_height - i
            if current_height >= check_height and check_height not in self.notified_heights:
                alert_content = self.format_alert(current_height, self.target_height)
    ...
    def monitor(self, check_interval=60):
        print(f"开始监控区块高度: {self.target_height}")
        print("按 Ctrl+C 停止监控")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("使用方法: python3 block_alert.py <目标区块高度>")
        sys.exit(1)
